...
- an explicit rejection of manual/automatic rebalance and provider-chained
  funding where one route spends another route's not-yet-known output; the only
  staged exception is a deterministic owned-ingress completion whose accepted
  asset and exact action are both frozen before commit;
...
| Disabled/unfundable route fails closed                                    | Disabled route/provider/capability, unknown fee/price, excess absolute or percentage cost, excess slippage, stale expiry, insufficient output, or provider-chained/post-receipt-discovered plan becomes unavailable or is rejected. A deterministic owned-ingress completion is valid only when its variant and action were frozen before commit. |
