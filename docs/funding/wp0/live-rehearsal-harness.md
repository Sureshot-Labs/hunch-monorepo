...
| `relay-wallet-solana-sol-to-pm`     | controlled native SOL → Relay Depository → PM settlement      | fresh one-instruction/one-ALT direct-deposit fixture, exact amount/order binding, shared Privy/on-chain outer boundary, fee reserve, PM readiness | finalized SOL debit, bounded deposit, destination pUSD/ready observation, no duplicate Buy | `FundingPlatformOwner`   |
