...
This historical Solana USDC run proves the quote/action/chain path only. It did
not prove a shared Privy Solana signer policy and must not be cited as delegated
native-SOL activation evidence. The required policy/custom-program proof now
lives in
...
| `relay-wallet-solana-sol-to-pm`     | controlled native SOL → Guard → Relay Depository → PM settlement | fresh direct-deposit fixture; Guard validates signed restricted Relay order, user binding, cap/replay/expiry, exact CPI and fee reserve; shared Privy policy permits only Guard + Secp256k1 + Compute Budget | finalized SOL debit, bounded deposit, destination pUSD/ready observation, no duplicate Buy or guard-held balance | `FundingPlatformOwner`   |
...
| `relay-strict-solana-sol-to-pm`     | external native SOL → strict Relay address → PM pUSD             | full signed order, exact user destination/refunds, amount/expiry, Solana address evidence                                                                                                                    | request/child correlation, destination pUSD or owned refund, no delegated signer                                 | `FinanceOperationsOwner` |
| `relay-strict-solana-usdc-to-pm`    | external canonical USDC → strict Relay address → PM pUSD         | frozen live-supported SPL fixture plus the same full-order and refund boundary                                                                                                                               | request/child correlation, destination pUSD or owned refund, no delegated signer                                 | `FinanceOperationsOwner` |
| `owned-solana-sol-receive`          | external native SOL → user's managed Solana wallet               | exact current wallet, pre-disclosure cursor, canonical native transfer identity                                                                                                                              | finalized SOL Account Value; no conversion operation or Buy continuation                                         | `FundingPlatformOwner`   |
...
## Original controlled-EVM Relay Deposit Address hard gates
...
Slice E2 is a separately gated strict Solana extension. It may accept a transfer
from an external wallet or exchange only because `refundTo` and every signed
order refund leg are independently fixed to a verified user-controlled wallet;
the exchange hot wallet is never inferred as the refund owner. E2 retains exact
amount, asset, expiry, child-request, and destination-credit requirements.
