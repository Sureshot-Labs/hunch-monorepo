...
function usage(exitCode = 1): never {
  console.error(`Usage: pnpm -C hunch-monorepo -F api run ai:map-search:smoke -- [options]
...
async function callXaiWithRetry(
  ...
  while (attempts < totalAttempts) {
    ...
    if (args.verbose) {
      console.log(
        `[ai-map-search-smoke] retry ${currentAttempt}/${totalAttempts - 1} after ${backoffMs}ms status=${raw.status} err=${raw.error ?? "unknown"}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  if (
    ...
    console.warn(
      "[ai-map-search-smoke] OPENROUTER_API_KEY missing; semantic routing will fallback to lexical only",
    );
  }
  ...
  await ensureRedis(redis, { waitForReady: true, logLabel: "ai-map-search-smoke" });
  ...
  console.log("[ai-map-search-smoke] start", {
    runId,
  ...
  while (queue.length > 0) {
    ...
    if (evidenceById.size >= args.maxEvidenceTotal) {
      console.log(
        `[ai-map-search-smoke] stop=max_evidence_total current=${evidenceById.size}`,
      );
      break;
    }
    ...
    while (launches.length < args.concurrency && queue.length > 0) {
      ...
      const promise = (async (): Promise<PreparedCallTask> => {
        ...
        console.log(
          `[ai-map-search-smoke] call_start #${launchOrder} node="${nodeDisplayLabel(node)}" level=${node.level} queue=${queue.length} evidence=${evidenceById.size} in_flight=${inFlightAtLaunch}`,
        );
    ...
    for (const task of results) {
      ...
      if (newEvidence.length > 0 && openRouterKey) {
        try {
          ...
          console.warn("[ai-map-search-smoke] evidence embedding failed", {
      ...
      if (args.verbose) {
        console.log("[ai-map-search-smoke] call", {
  ...
  if (args.out) {
    ...
    console.log(`[ai-map-search-smoke] wrote ${args.out}`);
  }
  if (args.reportOut) {
    await writeFile(args.reportOut, markdownReport, "utf8");
    console.log(`[ai-map-search-smoke] wrote ${args.reportOut}`);
  }
