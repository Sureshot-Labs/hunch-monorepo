...
function usage(exitCode = 1): never {
  console.error(`Usage: pnpm -C hunch-monorepo -F api run ai:map-signals:smoke -- [options]

Input:
  --in <path>                    Map-search smoke JSON (default: /tmp/ai-map-search-smoke.json)
...
async function readInput(path: string): Promise<MapSearchSmokeFile> {
  const raw = await readFile(path, "utf8");
  const parsed = JSON.parse(raw) as MapSearchSmokeFile;
  ...
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "X-Title": "Hunch AI Map Signals Smoke",
      },
      body: JSON.stringify({
        model: args.model,
        messages: [
...
function buildMarkdown(
  ...
  const lines: string[] = [];
  lines.push("# AI Map Signals Smoke Report");
  lines.push("");
...
async function main(): Promise<void> {
  ...
  await ensureRedis(redis, { waitForReady: true, logLabel: "ai-map-signals-smoke" });

  try {
    const startedAt = Date.now();

    console.log("[ai-map-signals-smoke] start", {
      runId,
    ...
    console.log("[ai-map-signals-smoke] plan", {
    ...
    async function enrichCandidateMarketTitles(
      ...
      try {
        ...
        console.warn("[ai-map-signals-smoke] market title enrichment failed", {
    ...
    const signals = await runParallel(buckets, args.concurrency, async (bucket, idx) => {
      ...
      inFlight += 1;
      console.log(
        `[ai-map-signals-smoke] call_start #${callIndex} node="${bucket.nodeLabel}" level=${bucket.level} queue=${Math.max(buckets.length - callIndex, 0)} evidence=${bucket.evidence.length} in_flight=${inFlight}`,
      );
      ...
      if (args.verbose) {
        console.log(
          `[ai-map-signals-smoke] call_ctx #${callIndex} candidates=${candidateMarkets.length} first_market=${candidateMarkets[0]?.marketId ?? "-"}`,
        );
      }
    ...
    if (args.outPath) {
      ...
      console.log(`[ai-map-signals-smoke] wrote ${args.outPath}`);
    }
