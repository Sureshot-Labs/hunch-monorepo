...
const ADMIN_SYSTEM_PRICE_REFRESH_VENUES: Partial<
  Record<AdminSystemVenue, PriceRefreshVenue>
> = {
...
async function buildAdminIndexerSystemStats() {
  ...
  const venues = await Promise.all(
    ADMIN_SYSTEM_VENUES.map(async (venue) => {
      try {
        const priceRefreshVenue = ADMIN_SYSTEM_PRICE_REFRESH_VENUES[venue];
          ...
          await Promise.all([
            ...
            priceRefreshVenue
              ? readRedisPriceRefreshQueueStats(
                  redis,
                  PRICE_REFRESH_QUEUE_KEYS[priceRefreshVenue],
                  nowMs,
                )
