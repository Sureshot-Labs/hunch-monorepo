  ...
  const { rows } = await pool.query<{
    ...
    `
      ...
      where user_id = $1
        ...
        and not (venue_order_id = any($3::text[]))
      order by posted_at desc nulls last, last_update desc nulls last
      limit 25
    `,
    [inputs.userId, inputs.signerAddress, inputs.openVenueOrderIds],
  );
