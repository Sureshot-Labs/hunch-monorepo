...
async function getOrphanedEvents(pool: Pool, eventIds: string[]): Promise<string[]> {
...
async function deleteClosedMarkets(pool: Pool, options: DeleteOptions): Promise<DeleteStats> {
  ...
  const statusCounts = closedMarkets.reduce((acc, m) => {
  ...
  if (options.dryRun) {
    ...
    closedMarkets.slice(0, 20).forEach((market, index) => {
      console.log(`${index + 1}. ${market.title} (${market.venue}) - Status: ${market.status}`);
    });
  ...
  for (const market of closedMarkets) {
    try {
      await pool.query('DELETE FROM unified_markets WHERE id = $1', [market.id]);
      deletedMarkets++;
      if (deletedMarkets % 100 === 0) {
        console.log(`  Deleted ${deletedMarkets}/${closedMarkets.length} markets...`);
      }
    } catch (error) {
...
async function main() {
  ...
  if (options.dryRun) {
    ...
    console.log('⚠️  WARNING: This will permanently delete all closed markets!');
  }
