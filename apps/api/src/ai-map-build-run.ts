...
function printHelp(): void {
  console.log(`Usage: pnpm -C hunch-monorepo -F api run ai:map-build:run -- [options]
...
All other args are passed through to ai:embed:market-map.
`);
}
...
async function main() {
  ...
  if (!env.redisUrl) {
    throw new Error("[map-build-run] REDIS_URL is required");
  }

  if (!config.enabled && !args.force) {
    console.log("[map-build-run] skipped (policy disabled)");
    return;
  }

  const redis = createRedisClient({ url: env.redisUrl });
  await ensureRedis(redis, { waitForReady: true, logLabel: "map-build-run" });
  ...
  try {
    ...
    if (acquired !== "OK") {
      console.log("[map-build-run] skipped (lock active)");
    ...
    if (!args.ignorePolicyRate) {
      if (lastRunMs > 0 && nowMs - lastRunMs < config.pollIntervalSec * 1_000) {
        console.log("[map-build-run] skipped (poll interval)");
        await setStatus(redis, {
          state: "skipped",
      ...
      if (runsInWindow.length >= config.maxRunsPerWindow) {
        console.log("[map-build-run] skipped (run window cap)");
        await setStatus(redis, {
          state: "skipped",
      ...
      if (runsInDay.length >= config.maxRunsPerDay) {
        console.log("[map-build-run] skipped (day run cap)");
        await setStatus(redis, {
          state: "skipped",
    ...
    if (!args.ignorePolicyBudget) {
      if (budgetWindowSpentUsd + estimatedCostUsd > config.budgetWindowUsd) {
        console.log("[map-build-run] skipped (budget window cap)");
        await setStatus(redis, {
          state: "skipped",
      ...
      if (daySpentUsd + estimatedCostUsd > config.dayBudgetUsd) {
        console.log("[map-build-run] skipped (day budget cap)");
        await setStatus(redis, {
          state: "skipped",
    ...
    if (args.verbose) {
      console.log("[map-build-run] executing", {
    ...
    try {
      ...
      if (!args.dryRun && providerReportedCostCalls === 0) {
        console.warn(
          `[map-build-run] warning runId=${runId} no provider-reported label cost; using estimated fallback`,
        );
      }
      ...
      console.log(
        `[map-build-run] done runId=${runId} state=${args.dryRun ? "dry_run" : "completed"} est_cost=${actualEstimatedCostUsd.toFixed(6)} charged_cost=${actualChargedCostUsd.toFixed(6)} source=${costSource}`,
      );
    } catch (error) {
      const finishedAt = Date.now();
      const runEntry: RunEntry = {
...
main()
  ...
  .catch(async (error) => {
    console.error("[map-build-run] failed", error);
    await pool.end();
    process.exit(1);
  });
