...
function validatePlannedSources(
  ...
  return sources.map((source) => {
    ...
    const polymarketRouterPreparationStepChain = isPolymarketRouterCommitPlan(
      source.commitPlan,
    );
    const composite =
    ...
    if (
      ...
      (source.option.kind !== "venue_preparation" ||
        ...
        !(
          ...
          polymarketRouterPreparationStepChain
        ) ||
        fundingEconomicSourceReservations(source.commitPlan.reservations)
          .length !== source.option.source.inputCount ||
    ...
    if (composite) {
      ...
      for (const leg of legs) {
        ...
        if (leg.source.kind === "venue_preparation") {
          ...
          const preparationReservations = fundingEconomicSourceReservations(
            source.commitPlan.reservations.filter(
