...
async function cleanupExpiredMarkets(pool: Pool, options: CleanupOptions): Promise<CleanupStats> {
  ...
  if (options.dryRun) {
    ...
    expiredMarkets.forEach((market, index) => {
      ...
      console.log(`${index + 1}. ${market.title} (${market.venue}) - Expired: ${expiryTime}`);
    });
  ...
  for (const market of expiredMarkets) {
    try {
      if (options.deleteMode) {
        ...
        updatedMarkets++;
        console.log(`📝 Updated market: ${market.title} (${market.venue}) -> CLOSED`);
      }
    } catch (error) {
