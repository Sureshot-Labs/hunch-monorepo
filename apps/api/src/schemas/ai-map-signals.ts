...
export function buildMapSignalsSystemPromptV2(): string {
  return [
    ...
    "Writing contract:",
    "- headline: 4-8 words when possible. It should read like a sharp market brief headline, not a taxonomy label, tag list, or internal note.",
    "- summary: usually 1 short sentence, or 2 only when needed. It should explain why this matters for the market right now.",
    "- headline + summary together should total 30-40 words.",
    "- Do not add filler just to hit the count.",
    "- Make every word serve the market implication.",
    ...
    "- Prefer this structure: headline signals the change; summary explains the market consequence.",
    "- Write so a trader can quickly judge whether this could change positioning now.",
    "- Prefer active market-aware verbs such as lifts, pressures, supports, weighs on, strengthens, weakens, narrows, widens, or tilts when they fit the evidence.",
    ...
    "- Avoid compressed analyst shorthand, generic journalism phrasing, academic phrasing, corporate tone, and filler.",
...
export function buildMapSignalsUserPromptV2(input: MapSignalsPromptInput): string {
  return [
    ...
    "- Write the visible copy like a compact market brief, not a news summary.",
    "- Keep headline + summary tight enough to scan in one glance.",
    "- Make the market consequence obvious, not implied.",
