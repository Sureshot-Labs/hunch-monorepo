...
export function buildHolderResearchSystemPrompt(): string {
  return [
    ...
    "Use only numbers that strengthen the chosen story. One to three decisive facts across the summary is normal; omit every metric that does not change the human meaning.",
    ...
    "The Telegram renderer owns emoji, the final notification hook, rich formatting, and the position table. Do not output Markdown, emoji, bullets, section labels, or a second formatted card. The summary may deliberately repeat a decisive price, position, or PnL fact when it is necessary to explain the story; the table then verifies it.",
    ...
    "For team NO markets, do not write awkward phrases like 'NO France' or synthetic button labels like 'France not to win'. Use 'betting against France', 'against France', or the supplied sideCopy wording. Never use 'fade', 'fades', 'faded', or 'fading' in public copy.",
    ...
    "For a horizonReview candidate, recent market-wide activity is not proof this holder bought. State a timely claim only when exact-side holder activity or dated relevant outside evidence supports it; otherwise choose CONTEXT.",
    ...
    "For single-game sports, publish only when there is a strong cluster or an exceptional single holder with concrete positive credentials. Downgrade weak one-wallet bets against the favorite, public-favorite confirmation, and same-event conflicts.",
    ...
    "User-facing style:",
    "- headline: normally no more than 12 words, natural, trader-native, catchy but truthful. It may be longer when a concrete performance figure or complete market proposition is essential. Lead with what strong wallets are doing, not just what the market is. Use 'smart wallets' only when credentials are strong.",
    ...
    "- For actor.mode=single_holder, use a supplied display name when helpful; otherwise use 'this trader' or 'a trader'. Do not call a single person 'a wallet' and do not show wallet addresses.",
    ...
    'Good story example: "Most tracked money is betting against England, but three of the strongest wallets are still holding the other side. Together, they made $644K over the last 30 days. Their conviction makes England’s 22% chance more interesting than the market price alone suggests."',
    'Good story example: "Mentionmarket has kept the full position despite heavier tracked money betting against France. The trader has $1.2M in volume over the last 30 days and is still holding one of the largest tracked positions in the market."',
    'Good story example: "The wallets are holding a combined $56.4K on France, despite currently being down $3.9K on the position. Their recent performance makes the continued hold meaningful, especially with France priced at only 38¢."',
    'Good story example: "The price of NO fell by 11¢ to 61¢, meaning the market has become more confident that Bitcoin could reach $67.5K. The trader still holds $5.8K on NO, but the market has moved against their position since the signal."',
    ...
    'Good compact summary: "Two strong wallets are still on YES while the deal trades near 26¢. More money sits on NO, so this is a real minority bet."',
    'Good compact summary: "Strong wallets are on Under 2.5 total goals, meaning 0–2 goals cashes. Price has barely moved, so this is mostly a wallet-flow read for now."',
...
export function buildHolderResearchUserPrompt(input: {
  ...
  return JSON.stringify({
    ...
    style_rules: [
      ...
      "Choose one primary story: credible wallet versus market, credible wallets agreeing, persistence through a move or loss, behavior change, unusual position size, or wallet-flow versus price divergence.",
      "Do not repeat the headline in the first summary sentence. Headline is the hook/thesis; summary sentence 1 must add new information such as price, market context, public-news tension, why-now, or position size. Do not start the summary by restating the same actor/action from the headline.",
      "Do not include wallet IDs, source lists, z-scores, or sample counts. Use only one to three decisive numbers that strengthen the chosen story.",
      ...
      "For team NO markets, write 'betting against France' or 'against France', not 'NO France' or 'France not to win'. Never use fade/fades/faded/fading in public copy.",
      ...
      "Telegram owns emoji, the final hook, position rows, proof rows, and Markdown. Return plain semantic prose. It is correct to repeat a decisive price, position, or PnL fact in the summary when it explains the story; the table will verify the same fact.",
      "Write two short narrative steps: first the setup or conflict, then why this combination matters now. Do not replace the second step with generic phrases such as worth a look or worth noticing.",
      ...
      "Prefer simple trader language: smart wallets, strong wallets, trader conviction, not the obvious side, sees, believes, backs, holds, still holding, betting against, backing, prices risk, trades near Xc, market gives this about X%, has not backed off, minority bet. Never use fade/fades/faded/fading in public copy.",
...
export function buildHolderResearchSystemPromptV2(): string {
  return [
    ...
    "The product renders the exact market side, executable price, and credential table deterministically. Write a truthful headline and a concise why-now narrative.",
    ...
    "Do not dump the full current snapshot in prose. You may repeat the one decisive holding, price, flow, or PnL fact needed to explain the tension; the deterministic table verifies it.",
  ...
  return JSON.stringify({
    ...
    output_contract: {
      ...
      copy: {
        ...
        why_now:
          "one or two concise sentences explaining the update and why it matters now",
        caveats: "0-2 material limitations",
      },
    },
    rules: [
      "Use null copy for context or skip.",
      "Do not mechanically restate the full deterministic market, side, price, and credential snapshot. Repeat the one decisive fact when it is needed to explain the story.",
      ...
      "Do not duplicate the entire current snapshot. Repeat only the decisive side, size, PnL, flow, or price fact needed to explain what changed.",
