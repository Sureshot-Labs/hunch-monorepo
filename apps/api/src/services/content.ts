...
async function promotePublishedSlug(
  db: DbQuery,
  articleId: string,
  nextSlug: string,
): Promise<string | null> {
  ...
  if (nextRows[0] && nextRows[0].article_id !== articleId) throw slugConflict();
...
function versionSnapshot(row: VersionRow): ContentArticleVersion["snapshot"] {
  return {
    schemaVersion: row.schema_version,
    slug: row.slug,
    title: row.title,
...
export async function publishContentArticle(
  ...
  return tx(pool, async (db) => {
    ...
    if (scheduled) {
      ...
      previousSlug = await promotePublishedSlug(db, inputs.id, version.slug);
      const now = new Date();
      await db.query(
        `
          update content_articles
...
export async function getContentArticleVersion(
  ...
  const row = rows[0];
  return row
    ? { ...versionSummary(row), snapshot: versionSnapshot(row) }
    : null;
}
