...
async function insertContentAssetAudit(
  ...
  await db.query(
    ...
    [
      input.action,
      input.assetId,
      contentActorAdminId(input.actor),
      input.actor.kind,
...
function storageConfigured(): boolean {
  return getContentServiceRuntime().assetStorageConfigured;
}
...
function requireStorage(): S3Client {
  ...
  const staticCredentials = config.assetStaticCredentialsConfigured
    ? {
        credentials: {
          accessKeyId: config.assetS3AccessKeyId,
          secretAccessKey: config.assetS3SecretAccessKey,
        },
      }
    : {};
  storageClient ??= new S3Client({
    ...
    forcePathStyle: config.assetS3ForcePathStyle,
    ...staticCredentials,
  });
...
function publicUrl(key: string): string {
  return `${getContentServiceRuntime().assetPublicBaseUrl}/${key
    .split("/")
...
function copySource(key: string): string {
  return [getContentServiceRuntime().assetS3Bucket, ...key.split("/")]
    .map(encodeURIComponent)
    .join("/");
}
...
async function readObjectPrefix(
  ...
  const object = await client.send(
    new GetObjectCommand({
      Bucket: getContentServiceRuntime().assetS3Bucket,
      Key: key,
