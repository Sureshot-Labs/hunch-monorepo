...
export type WalletIntelRefreshPolicy = {
  ...
  onchainStateLimit: number;
  onchainStateStaleHours: number;
  onchainStateErrorStaleHours: number;
};
...
const walletIntelRefreshSchema = z
  .object({
    ...
    onchainStateLimit: nonNegativeInt,
    onchainStateStaleHours: positiveInt,
    onchainStateErrorStaleHours: positiveInt,
  })
  .strict()
  .partial();
...
const holderResearchSchema = z
  .object({
    ...
    backgroundContextEnabled: strictBoolean,
...
function getDefaults(): IntelPolicyMap {
  return {
    ...
    holder_research: {
      ...
      externalSearchMaxOutputTokens: 2_400,
      externalSearchMaxTurns: 8,
...
function normalizeRefreshPolicy(
  ...
  return {
    ...
    onchainStateLimit: Math.max(0, Math.trunc(policy.onchainStateLimit)),
    onchainStateStaleHours: Math.max(
      1,
      Math.trunc(policy.onchainStateStaleHours),
    ),
    onchainStateErrorStaleHours: Math.max(
      1,
      Math.trunc(policy.onchainStateErrorStaleHours),
    ),
  };
}
...
function normalizeHolderResearchPolicy(
  ...
  return {
    ...
    backgroundContextEnabled: Boolean(policy.backgroundContextEnabled),
    ...
    model: policy.model.trim() || "openai/gpt-6-sol",
