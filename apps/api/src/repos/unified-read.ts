...
export type FeedMarketQueryPhase = "market_rank_prefix" | "market_scope_filter";

type FeedMarketFastPathOptions = {
  acceptPartialMetricPage?: boolean;
  preRankEventFilterInputs?: FeedEventFilterInputs;
  onPhase?: (phase: FeedMarketQueryPhase) => void;
};
...
const FEED_PROJECTED_METRIC_CANDIDATE_LIMIT = 4000;
      ...
      ? (() => {
          ...
          return `
            ...
            probability_grace_market_ids as materialized (
              select array_agg(distinct venue_market_id) as venue_market_ids
              from probability_grace_markets
            ),
            probability_grace_availability as materialized (
              select pm_filter.id
              from polymarket_markets pm_filter
              cross join probability_grace_market_ids grace_market_ids
              where pm_filter.id = any(grace_market_ids.venue_market_ids)
                and pm_filter.accepting_orders = true
                and coalesce(pm_filter.active, true) = true
                and coalesce(pm_filter.closed, false) = false
                and coalesce(pm_filter.archived, false) = false
            )
            select
              strict_market.market_id,
              strict_market.token_yes,
              strict_market.token_no,
              strict_market.clob_token_ids
            from probability_strict_markets strict_market
            union all
            select
              grace_market.market_id,
              grace_market.token_yes,
              grace_market.token_no,
              grace_market.clob_token_ids
            from probability_grace_markets grace_market
...
function buildFeedMarketCandidateExtraSql(args: {
  ...
  if (marketIdsParam) {
    clauses.push(`${marketAlias}.id = ANY(${marketIdsParam}::text[])`);
  }
  if (hasSearch) {
...
async function fetchFeedChange24hEventIdsFast(
  ...
  const hasMarketQualificationFilter =
    Boolean(inputs.marketIds?.length) ||
    inputs.maxSpread != null ||
    inputs.minLiquidity > 0 ||
    Boolean(inputs.durationMinutes?.length);
  if (
    inputs.sort !== "change24h" ||
    inputs.eventScope ||
    inputs.minProb != null ||
    inputs.maxProb != null ||
    (inputs.minVol > 1e-9 && hasMarketQualificationFilter) ||
  ...
  for (const candidateLimit of FEED_CHANGE24H_V2_CANDIDATE_LIMITS) {
    ...
    const expressions = buildFeedSqlExpressions();
    const nowParam = add(inputs.nowParam);
    const selectedMarketIdsParam = inputs.marketIds?.length
      ? add(inputs.marketIds)
      : null;
    const marketQual: string[] = [];
    if (selectedMarketIdsParam) {
      marketQual.push("om.id in (select market_id from selected_market_ids)");
    }
    const durationSql = buildMarketDurationSql(inputs, add, "om");
    if (durationSql) marketQual.push(durationSql);
    if (inputs.minLiquidity > 0) {
      marketQual.push(
    ...
    if (inputs.maxSpread != null) {
      marketQual.push(
        ...
        `(om.best_ask - om.best_bid) <= ${add(inputs.maxSpread)}`,
      );
    }
    ...
    eventWhere.push(
      buildEventHasOrderableMarketSql({
        eventAlias: "e",
        nowParam,
        extraMarketSql: marketQual,
      }),
    );
    if (inputs.minVol > 1e-9) {
      eventWhere.push(
    ...
    const selectedMarketIdsCte = selectedMarketIdsParam
      ? `
        selected_market_ids as materialized (
          select market_id
          from unnest(${selectedMarketIdsParam}::text[]) as selected_market(market_id)
        ),
      `
      : "";
    const rows = await queryRowsWithLocalSettings<{
      ...
      pool,
      `
        with ${selectedMarketIdsCte}
        change24h_v2_ranked_events as materialized (
        ...
        valid_ranked_events as materialized (
          select ranked_event.event_id, ranked_event.change_24h
          from unified_events e
        ...
        select
          ...
          (select count(*)::int from change24h_v2_ranked_events) as candidate_count
      `,
      params,
      {
        workMem: FEED_HEAVY_QUERY_WORK_MEM,
    ...
    const state = rows[0];
    const ids = state?.ids ?? [];
    const candidateCount = Number(state?.candidate_count ?? 0);
    const eventRows = ids.map((id) => ({
      id,
      cached_change_24h: true,
    }));
...
function buildFeedMarketViewContext(args: {
  ...
  const { supportedLimitlessMarketExpr, renderableMarketExpr } = expressions;
  ...
  const where = buildFeedMarketPostCandidateWhere({
    add,
    inputs,
    expressions,
  });
...
function buildFeedMarketPostCandidateWhere(args: {
  add: PgParamAdder;
  expressions: FeedSqlExpressions;
  inputs: Pick<
    FeedInputs,
    "maxProb" | "maxSpread" | "minLiquidity" | "minProb" | "minVol"
  >;
}): string[] {
  const { add, expressions, inputs } = args;
  const { marketLiquidityDisplayExpr, marketVolumeDisplayExpr } = expressions;
  ...
  return where.length ? where : ["true"];
}
...
async function fetchFeedPreselectedMarketIdsFast(
  ...
  const { params, add } = createParamBuilder();
  const expressions = buildFeedSqlExpressions();
  const { marketVolumeDisplayExpr, marketLiquidityDisplayExpr } = expressions;
  const nowParam = add(inputs.nowParam);
  const nowCloseParam = add(inputs.nowParam);
  const marketContext = buildFeedMarketViewContext({
    add,
  ...
  const limitParam = add(inputs.limit);
  const offsetParam = add(inputs.offset);
  const withParts = [marketContext.orderableMarketCandidatesCte];
  if (marketContext.scopedOrderableMarketCandidatesCte) {
    withParts.push(marketContext.scopedOrderableMarketCandidatesCte);
  }

  let marketOrder: string;
  let metricJoin = "";
  if (inputs.sort === "totalvol") {
    ...
    metricJoin = `
      left join unified_market_trade_24h trade_24h
        on trade_24h.market_id = m.id
    `;
    ...
    marketOrder = `
      ...
      end ${sortDir} nulls last, m.venue_market_id
    `;
  } else {
    marketOrder = `
      (coalesce(${marketVolumeDisplayExpr}, 0) * 0.4 +
       coalesce(${marketLiquidityDisplayExpr}, 0) * 0.3 +
       case when e.start_date >= ${add(inputs.sevenDaysAgo)}::timestamptz then 1000 else 0 end * 0.2 +
       case when e.end_date > ${nowParam}::timestamptz and e.end_date <= ${add(inputs.sevenDaysFromNow)}::timestamptz then 500 else 0 end * 0.1
...
async function fetchFeedChange24hMarketIdsFast(
  ...
  for (const candidateLimit of FEED_CHANGE24H_V2_CANDIDATE_LIMITS) {
    ...
    let marketCandidateCtes: string;
    let marketCandidateFrom: string;
    let marketCandidateWhere: string[];
    if (inputs.eventScope) {
      const scopeMarketExtraSql = buildFeedMarketCandidateExtraSql({
        add,
        inputs,
        nowParam,
        venueTarget: "market",
        eventAlias: "scope_event",
      ...
      const candidateMarketExtraSql = buildFeedMarketCandidateExtraSql({
        add,
        inputs,
        nowParam,
        venueTarget: "market",
      ...
      const postCandidateWhere = buildFeedMarketPostCandidateWhere({
        add,
        inputs,
        expressions,
      });
      marketCandidateCtes = `
        change24h_v2_candidate_events as materialized (
          select distinct candidate_market.event_id as id
          from ${candidateCteName} ranked_cache
          join unified_markets candidate_market
            on candidate_market.id = ranked_cache.market_id
        ),
        change24h_v2_candidate_event_scope as materialized (
          select
            candidate_event.id,
            (
              select count(*)::int
              from (
                select scope_market.id
                from unified_markets scope_market
                join unified_events scope_event
                  on scope_event.id = scope_market.event_id
                left join polymarket_markets scope_pm
                  on scope_pm.id = scope_market.venue_market_id
                 and scope_market.venue = 'polymarket'
                where scope_market.event_id = candidate_event.id
                  and ${buildBroadOrderableMarketSql({
                    marketAlias: "scope_market",
                    eventAlias: "scope_event",
                    nowParam,
                  ...
                  and ${scopeMarketExtraSql.join(" and ")}
                limit 2
              ) qualified_scope_market
            ) as market_count
          from change24h_v2_candidate_events candidate_event
        ),
        change24h_v2_valid_candidate_events as materialized (
          select candidate_event_scope.id
          from change24h_v2_candidate_event_scope candidate_event_scope
          where candidate_event_scope.market_count ${inputs.eventScope === "grouped" ? "> 1" : "= 1"}
        )
      `;
      marketCandidateFrom = `
        ${candidateCteName} ranked_cache
        join unified_markets m on m.id = ranked_cache.market_id
        join unified_events e on e.id = m.event_id
        join change24h_v2_valid_candidate_events valid_candidate_event
          on valid_candidate_event.id = m.event_id
        left join polymarket_markets candidate_pm
          on candidate_pm.id = m.venue_market_id
         and m.venue = 'polymarket'
      `;
      marketCandidateWhere = [
        buildBroadOrderableMarketSql({
          marketAlias: "m",
          eventAlias: "e",
          nowParam,
          nowCloseParam,
          pmAlias: "candidate_pm",
        }),
        ...candidateMarketExtraSql,
        ...postCandidateWhere,
      ];
    } else {
      const marketContext = buildFeedMarketViewContext({
      ...
      marketCandidateCtes = marketContext.orderableMarketCandidatesCte;
      marketCandidateFrom = `
        ${marketContext.orderableMarketCandidateSource} candidate_market
        join unified_markets m on m.id = candidate_market.market_id
        join unified_events e on e.id = candidate_market.event_id
        join ${candidateCteName} ranked_cache
          on ranked_cache.market_id = m.id
      `;
      marketCandidateWhere = marketContext.where;
    }
    const limitParam = add(inputs.limit);
    const offsetParam = add(inputs.offset);
    const candidateLimitParam = add(candidateLimit);
...
async function filterRankedFeedMarketCandidates(
  ...
  let eventScopeJoin = "";
  if (inputs.eventScope) {
    const expressions = buildFeedSqlExpressions("scope_market", "scope_event");
    const nowParam = add(inputs.nowParam);
    const scopeMarketExtraSql = buildFeedMarketCandidateExtraSql({
      add,
      inputs,
    ...
    eventScopeCtes = `,
      selected_event_scope as materialized (
        select candidate_event.id as event_id, scoped_market.market_count
        from selected_event_candidates candidate_event
        join lateral (
          select count(*)::int as market_count
          from (
            select scope_market.id
            from unified_markets scope_market
            join unified_events scope_event
...
async function fetchProgressivelyFilteredFeedMarketIds(
  pool: Pool,
  inputs: FeedInputs,
  options?: Pick<FeedMarketFastPathOptions, "onPhase">,
): Promise<string[] | null> {
  const pageTarget = inputs.limit + inputs.offset;
  ...
  options?.onPhase?.("market_rank_prefix");
  ...
  options?.onPhase?.("market_scope_filter");
...
async function fetchFeedProjectedMetricMarketIdsFast(
  pool: Pool,
  inputs: FeedInputs,
  ...
  if (inputs.minProb != null || inputs.maxProb != null) return null;
  const eventFilterInputs = options?.preRankEventFilterInputs ?? inputs;
  if (
    eventFilterInputs.category ||
    eventFilterInputs.categories?.length ||
    eventFilterInputs.filter ||
    eventFilterInputs.endWithin ||
    eventFilterInputs.ageSince
  ) {
    return null;
  }

  const { params, add } = createParamBuilder();
  const metricSortValue =
    inputs.sort === "totalvol"
      ? "metric.volume_total_now"
      : inputs.sort === "liquidity"
        ? "coalesce(nullif(metric.liquidity_now, 0), nullif(metric.open_interest_now, 0))"
        : "metric.open_interest_now";
  const metricEligibleSql =
    inputs.sort === "totalvol"
      ? "metric.volume_total_now > 0"
      : `(${metricSortValue}) is not null`;
  const sortDir = inputs.sortDir === "asc" ? "asc" : "desc";
  const venueParam = inputs.venues?.length ? add(inputs.venues) : null;
  const candidateLimitParam = add(FEED_PROJECTED_METRIC_CANDIDATE_LIMIT);
  const rows = await queryRowsWithLocalSettings<FeedMarketIdPageRow>(
    pool,
    `
      with projected_metric_prefix as materialized (
        select metric.market_id, (${metricSortValue}) as sort_value
        from unified_market_activity_metrics_24h metric
        where ${metricEligibleSql}
          ${venueParam ? `and metric.venue = ANY(${venueParam}::text[])` : ""}
        order by sort_value ${sortDir} nulls last, metric.market_id
        limit ${candidateLimitParam}
      )
      select coalesce(
        array_agg(market_id order by sort_value ${sortDir} nulls last, market_id),
        '{}'::text[]
      ) as ids
      from projected_metric_prefix
    `,
    params,
    {
      workMem: FEED_HEAVY_QUERY_WORK_MEM,
  ...
  const candidateMarketIds = rows[0]?.ids ?? [];
  if (candidateMarketIds.length === 0) return [];

  // The hourly metrics relation is a compact rank source, not the response
  // source. Re-apply every live market/event predicate and the requested sort
...
async function fetchFeedMarketIdsFast(
  ...
  const projectedMetricIds = await fetchFeedProjectedMetricMarketIdsFast(
    pool,
    inputs,
    options,
  );
  if (projectedMetricIds != null) return projectedMetricIds;

  if (probabilityPredicate) return null;

  const sortDir = inputs.sortDir === "asc" ? "asc" : "desc";
  const projectedSortDir =
    inputs.filter === "endingsoon"
      ? "asc"
      : inputs.filter === "newest"
        ? "desc"
  ...
  if (inputs.sort === "totalvol") {
    ...
    projectedSortValue = "candidate_market.event_start_date";
  } else if (inputs.filter === "endingsoon") {
    projectedSortValue = `case
      when candidate_market.event_end_date is not null
       and candidate_market.event_end_date > ${nowParam}::timestamptz
        then candidate_market.event_end_date
      else null
    end`;
  } else {
  ...
  const shouldPreRankEvents = Boolean(
  ...
  const preRankEventCte =
    shouldPreRankEvents && needsPreRankEventFilter
      ? (() => {
          const eventWhere = buildFeedEventWhere({
            add,
            inputs: {
  ...
  const sql = `
    ...
    ranked_market_page as materialized (
      ...
      order by sort_value ${projectedSortDir} nulls last,
        candidate_market.venue_market_id
      limit ${limitParam}
      offset ${offsetParam}
    )
    select
      coalesce(
        array_agg(
          market_id
          order by sort_value ${projectedSortDir} nulls last, venue_market_id
        ),
        '{}'::text[]
      ) as ids,
      (
...
export async function fetchFeedMarketIdsForProbabilityProbe(
  ...
  const rankedMarketIds =
    fastMarketIds ??
    (needsBoundedPostFilter && rankedInputs.sort !== "change24h"
      ? []
      : (await fetchFeedMarketsDirect(pool, rankedInputs)).map(
          (candidateRow) => candidateRow.market_uuid,
        ));
  let marketIds = rankedMarketIds;
  if (needsBoundedPostFilter) {
    options?.onPhase?.("market_scope_filter");
    marketIds = await filterRankedFeedMarketCandidates(
      pool,
      probabilityFreeInputs,
      rankedMarketIds,
    );
...
export async function fetchFeedMarketsDirect(
  ...
  if (!preselectedMarketIds) {
    ...
    if (
      // change24h keeps its existing exact rollout fallback until the v2
      // cache is populated; other bounded scope/category paths must not
      // re-enter the multi-gigabyte exact query after declining fast rank.
      inputs.sort !== "change24h" &&
      !buildFeedSearchPlan(inputs.q).hasSearch &&
      (inputs.eventScope || hasFeedMarketCategoryFilter(inputs))
    ) {
      return [];
    }
  }
