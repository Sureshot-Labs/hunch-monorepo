...
/** Shared by account display and editable amount estimates, never execution. */
export const accountValueDisplayPriceAdapters = [
  pythSolUsdPriceAdapter,
] as const;
...
export function estimateNativeSolUsd(raw: string): Promise<UsdEstimate | null> {
  return pythSolUsdPriceAdapter.freshValue({
    amount: { asset: SOLANA_NATIVE_ASSET, raw },
