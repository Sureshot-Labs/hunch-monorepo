...
export class AuthService {
  ...
  static async createOrUpdateVenueCredentials(
    ...
    const result = await pool.query(
      ...
      [userId, walletAddress, venue, apiKey, apiSecret, additionalData ? JSON.stringify(additionalData) : null]
    );
  ...
  static async getVenueCredentials(
    ...
    let query = 'SELECT id, user_id, wallet_address, venue, api_key, api_secret, additional_data, is_active, created_at, updated_at, last_used_at FROM user_venue_credentials WHERE user_id = $1 AND venue = $2 AND is_active = true';
    let params = [userId, venue];
   ...
   */
  static async getAllVenueCredentials(userId: string, walletAddress?: string): Promise<VenueCredentials[]> {
    let query = 'SELECT id, user_id, wallet_address, venue, api_key, api_secret, additional_data, is_active, created_at, updated_at, last_used_at FROM user_venue_credentials WHERE user_id = $1 AND is_active = true';
    let params = [userId];
  ...
  static async createOrUpdatePolymarketCredentials(
    ...
    return this.createOrUpdateVenueCredentials(userId, walletAddress, 'polymarket', apiKey, apiSecret);
  }

  static async getPolymarketCredentials(userId: string, walletAddress?: string): Promise<PolymarketCredentials | null> {
...
export function createAuthMiddleware() {
  return async (request: any, reply: any) => {
    ...
    (request as any).walletAddress = decoded.walletAddress;
