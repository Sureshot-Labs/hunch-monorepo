    ...
    if (
      ...
      assert.ok(
        Array.isArray(params[0]) &&
          ...
          params[0].includes("0215_telegram_buy_delivery_modes.sql") &&
...
function buildTelegram0201Db(
  ...
  return {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      if (normalized.includes("from pg_constraint")) {
        if (
          params[0] === "public.telegram_trade_intents" &&
          params[1] === "telegram_trade_intents_delivery_authority_check"
        ) {
          return { rows: [] };
        }
  ...
  return {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      if (normalized.includes("from pg_constraint")) {
        ...
        if (
          params[0] === "public.telegram_trade_intents" &&
          constraint === "telegram_trade_intents_delivery_authority_check"
        ) {
          return { rows: [] };
        }
