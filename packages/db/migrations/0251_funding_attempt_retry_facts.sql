-- Permit only the two existing bounded Relay retry facts. This changes no
-- historical rows: execution evidence and all other finished-attempt fields
-- remain immutable. Eligibility is checked only when a retry fact is written.
