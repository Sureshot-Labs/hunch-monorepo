-- A receive receipt may be routed again only after its prior child operation
-- is terminal and explicitly detached. Preserve every reservation generation
-- by operation while allowing that audited retry lineage to share one receipt.
...
create index if not exists
  telegram_funding_authorization_reservations_receipt_idx
  on telegram_funding_authorization_reservations (receive_receipt_id);

-- A finalized receive source credit remains one physical observation. During
-- an explicit retry, move that allocation to the new child rather than
-- duplicating the chain event. The trigger admits only an exact receipt-linked
-- terminal-child handoff with either a resolved Relay reservation or proof
-- that the prior child could not have broadcast.
