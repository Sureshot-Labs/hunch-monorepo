-- Validate preparation-local dependencies independently of provider step ordinals.
-- Replace the validator only; do not scan or mutate historical operations.
  ...
  if supports_unbound_preparation_chain then
    ...
    where (
      ...
      or (
        ...
        and preparation_step.preparation_ordinal <> unbound_step_count - 1
      )
    );
