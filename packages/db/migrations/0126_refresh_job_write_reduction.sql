...
BEGIN
  ...
  changed_aggregated AS MATERIALIZED (
    SELECT a.*
    FROM aggregated a
    LEFT JOIN unified_event_activity_snapshots_1h existing
      ON existing.event_id = a.event_id
     AND existing.venue = a.venue
     AND existing.bucket = a.bucket
    WHERE existing.event_id IS NULL
       OR existing.volume_total IS DISTINCT FROM a.volume_total
       OR existing.liquidity IS DISTINCT FROM a.liquidity
       OR existing.open_interest IS DISTINCT FROM a.open_interest
       OR existing.source_updated_at IS DISTINCT FROM a.source_updated_at
  )
