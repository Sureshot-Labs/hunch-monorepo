-- Recognize both existing Polymarket custody envelopes without changing old rows.
-- Exact owner, token, amount and calls remain immutable backend-validated facts.
-- Only functions are replaced: historical operations are not checked at deploy.
create or replace function funding_is_owned_controller_handoff(
  funding_executor_id text,
  funding_action jsonb
)
returns boolean
language sql
immutable
as $$
  select coalesce(
    funding_action ->> 'kind' = 'external_handoff'
    and (
      (funding_executor_id = 'polymarket_deposit_wallet_relayer_v1'
        and funding_action ->> 'handoffKind' = 'polymarket_deposit_wallet_transfer'
        and funding_action -> 'payload' ->> 'topology' = 'deposit_wallet')
      or (funding_executor_id = 'polymarket_safe_relayer_v1'
        and funding_action ->> 'handoffKind' = 'polymarket_safe_transfer'
        and funding_action -> 'payload' ->> 'topology' = 'safe')
    ), false);
$$;
