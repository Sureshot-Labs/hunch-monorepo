-- Composite funding may combine one exact venue-preparation contribution with
-- one or more exact Relay segments. Preparation remains unbound because it is
-- destination-local; every provider action and reservation remains bound to
-- its immutable provider segment.
...
declare
  ...
  invalid_preparation_step_binding_count integer;
...
begin
  ...
  select
    ...
    count(*) filter (
      where
        (step_kind = 'venue_preparation' and segment_id is not null)
        or
        (step_kind <> 'venue_preparation' and segment_id is null)
    )::integer,
    min(ordinal)::integer,
    max(ordinal)::integer
  into
    ...
    invalid_preparation_step_binding_count,
  ...
  if invalid_preparation_step_binding_count <> 0 then
    raise exception 'only venue-preparation steps may be unbound'
      using errcode = '23514';
  end if;
  ...
  if operation_plan_kind = 'composite_route' then
    ...
    if unbound_step_count <> venue_preparation_step_count then
      raise exception 'composite unbound steps must be exact venue preparation'
        using errcode = '23514';
    end if;
    ...
    where observation.operation_id = target_operation_id
      and (
        (
          ...
          and (
            observation.segment_id is not null
            or (
