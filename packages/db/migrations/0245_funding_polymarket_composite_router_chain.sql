-- Keep the canonical Polymarket Router chain valid when it is one unbound
-- venue-preparation contributor inside a composite route. Provider legs stay
-- segment-bound; handoff, approvals, and Router fund stay one exact linear
-- unbound chain. This aligns the durable constraint with the planner contract.
...
declare
  ...
  router_chain_step_count integer;
...
begin
  ...
  select
    ...
    count(*) filter (
      where funding_step.segment_id is null
        and is_polymarket_router_operation
        and funding_step.step_kind = 'transaction'
        and funding_step.normalized_action ->> 'kind' = 'evm_transaction'
        and funding_step.action_validation_result ->> 'validatorId'
          = 'polymarket_funding_router_v1'
        and funding_step.action_validation_result -> 'valid' = 'true'::jsonb
    ...
    count(*) filter (
      where
        ...
        or (
          ...
          and not (
            is_polymarket_router_operation
            and funding_step.step_kind = 'transaction'
            and funding_step.normalized_action ->> 'kind' = 'evm_transaction'
            and funding_step.action_validation_result ->> 'validatorId'
              = 'polymarket_funding_router_v1'
        ...
        where dependent_step.operation_id = pre_route_step.operation_id
          ...
          and (
            ...
            or (
              ...
              and not is_polymarket_router_operation
  ...
  if operation_plan_kind = 'venue_preparation'
    and step_count = 1 then
    if venue_preparation_step_count <> 1 then
      raise exception 'venue preparation plan requires one exact preparation step'
        using errcode = '23514';
    end if;
  elsif is_polymarket_router_operation then
      router_chain_step_count := unbound_step_count;
      if router_chain_step_count < 1
        or router_chain_step_count > 4
        or venue_preparation_step_count <> 1
        or pre_route_handoff_step_count > 1
        or router_approval_step_count
          <> router_chain_step_count - venue_preparation_step_count - pre_route_handoff_step_count then
      ...
      where funding_step.operation_id = target_operation_id
        ...
        and (
          (
            funding_step.step_kind = 'venue_preparation'
            and funding_step.ordinal <> router_chain_step_count - 1
          )
          or (
            funding_step.step_kind = 'external_handoff'
            and (
          ...
          or (
            ...
            and (
              ...
              or funding_step.action_validation_result -> 'valid'
  ...
  elsif operation_plan_kind = 'venue_preparation' then
  ...
  if operation_plan_kind = 'composite_route' then
    ...
    if (
      ...
      not is_polymarket_router_operation
