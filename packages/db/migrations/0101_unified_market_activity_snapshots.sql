...
CREATE TABLE IF NOT EXISTS unified_market_activity_snapshots_1h (
...
CREATE INDEX IF NOT EXISTS idx_unified_market_activity_snapshots_bucket
  ON unified_market_activity_snapshots_1h (bucket DESC);

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_snapshots_event_bucket
  ON unified_market_activity_snapshots_1h (event_id, bucket DESC);

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_snapshots_venue_bucket
  ON unified_market_activity_snapshots_1h (venue, bucket DESC);

CREATE TABLE IF NOT EXISTS unified_market_activity_metrics_24h (
  market_id text PRIMARY KEY,
  event_id text NOT NULL,
  venue text NOT NULL,
  volume_total_now numeric,
  volume_total_24h_ago numeric,
  volume_total_48h_ago numeric,
  volume_last_24h numeric,
  volume_prev_24h numeric,
  volume_last_24h_change numeric,
  volume_last_24h_change_pct numeric,
  liquidity_now numeric,
  liquidity_24h_ago numeric,
  liquidity_change_24h numeric,
  liquidity_change_pct_24h numeric,
  open_interest_now numeric,
  open_interest_24h_ago numeric,
  open_interest_change_24h numeric,
  open_interest_change_pct_24h numeric,
  has_24h_window boolean NOT NULL DEFAULT false,
  has_48h_window boolean NOT NULL DEFAULT false,
  volume_valid boolean NOT NULL DEFAULT false,
  liquidity_valid boolean NOT NULL DEFAULT false,
  open_interest_valid boolean NOT NULL DEFAULT false,
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_event
  ON unified_market_activity_metrics_24h (event_id);

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_venue
  ON unified_market_activity_metrics_24h (venue);

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_volume
  ON unified_market_activity_metrics_24h (volume_last_24h DESC)
  WHERE volume_valid;

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_volume_change_pct
  ON unified_market_activity_metrics_24h (volume_last_24h_change_pct DESC)
  WHERE volume_valid;

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_liquidity_change_pct
  ON unified_market_activity_metrics_24h (liquidity_change_pct_24h DESC)
  WHERE liquidity_valid;

CREATE INDEX IF NOT EXISTS idx_unified_market_activity_metrics_open_interest_change_pct
  ON unified_market_activity_metrics_24h (open_interest_change_pct_24h DESC)
  WHERE open_interest_valid;

CREATE TABLE IF NOT EXISTS unified_event_activity_metrics_24h (
...
CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_venue
  ON unified_event_activity_metrics_24h (venue);

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_volume
  ON unified_event_activity_metrics_24h (volume_last_24h DESC)
  WHERE volume_valid;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_venue_volume
  ON unified_event_activity_metrics_24h (venue, volume_last_24h DESC)
  WHERE volume_valid AND volume_last_24h IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_volume_change_pct
  ON unified_event_activity_metrics_24h (volume_last_24h_change_pct DESC)
  WHERE volume_valid;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_venue_volume_change_pct
  ON unified_event_activity_metrics_24h (
    venue,
    volume_last_24h_change_pct DESC,
    volume_last_24h_change DESC,
    volume_last_24h DESC
  )
  WHERE volume_valid AND volume_last_24h_change_pct IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_liquidity_change_pct
  ON unified_event_activity_metrics_24h (liquidity_change_pct_24h DESC)
  WHERE liquidity_valid;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_venue_liquidity_change_pct
  ON unified_event_activity_metrics_24h (
    venue,
    liquidity_change_pct_24h DESC,
    liquidity_change_24h DESC,
    liquidity_now DESC
  )
  WHERE liquidity_valid AND liquidity_change_pct_24h IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_unified_event_activity_metrics_open_interest_change_pct
  ON unified_event_activity_metrics_24h (open_interest_change_pct_24h DESC)
  WHERE open_interest_valid;
...
BEGIN
  DELETE FROM unified_market_activity_snapshots_1h
  WHERE bucket < v_now - interval '90 days';
...
LANGUAGE SQL
AS $$
  SELECT refresh_unified_market_activity_metrics_1h()
$$;
    ...
    BEGIN
      IF NOT EXISTS (
        ...
        WHERE proc_name IN (
          ...
          'refresh_unified_market_activity_metrics_1h_job'
        )
      ) THEN
        PERFORM add_job(
          'refresh_unified_market_activity_metrics_1h_job',
