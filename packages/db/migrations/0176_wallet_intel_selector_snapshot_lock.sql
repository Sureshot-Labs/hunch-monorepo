...
LANGUAGE SQL
AS $$
  -- Serialize selector writers with the existing hourly wallet-intel run lock.
...
AS $$
BEGIN
  -- Do not queue behind a long hourly refresh. The next five-minute job retries.
  IF NOT pg_try_advisory_xact_lock(4207, 1) THEN
    RAISE LOG 'refresh_wallet_intel_selector_snapshot_job skipped: wallet intel refresh active';
    RETURN;
  END IF;

  PERFORM refresh_wallet_intel_selector_snapshot();
END;
$$;
